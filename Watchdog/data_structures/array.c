...
ARY_ErrorState ARY_CheckError(volatile struct Array* ary){
...
ARY_ErrorState ARY_ResetError(volatile struct Array* ary){
	if (ary == NULL) return ARRAY_ERROR_NULL_ARRAY;
...
static OPSTATE ARY_GrowCapacity(volatile struct Array* ary){
	char* temp;
...
OPSTATE ARY_InitArray(volatile struct Array* ary, int typeSize){
...
OPSTATE ARY_InitArrayAdv(volatile struct Array* ary, int typeSize, const void* defValue, int size){
...
OPSTATE ARY_CloseArray(volatile struct Array* ary){
	if (ary == NULL) return OPSTATE_FAILURE;
...
OPSTATE ARY_CloneArray(volatile struct Array* original, volatile struct Array* clone){
...
char ARY_IsEmpty(volatile struct Array* ary){
	if (ARY_CheckError(ary)) return 1; // An errored array will always be treated as empty.
	return ary->size == 0;
}

OPSTATE ARY_Add(volatile struct Array* ary, const void* value){
...
OPSTATE ARY_Remove(volatile struct Array* ary, void* value){
	if (ARY_CheckError(ary)) return OPSTATE_FAILURE; // If error, return
	if (ARY_IsEmpty(ary))
		return OPSTATE_FAILURE;
...
OPSTATE ARY_Insert(volatile struct Array* ary, int index, const void* value){
	if (ARY_CheckError(ary)) return OPSTATE_FAILURE; // If error, return
...
OPSTATE ARY_RemoveAt(volatile struct Array* ary, int index, void* value){
	if (ARY_CheckError(ary)) return OPSTATE_FAILURE; // If error, return
	if (ARY_IsEmpty(ary)){
		return OPSTATE_FAILURE;
	}
...
int ARY_IndexOf(volatile struct Array* ary, const void* value){
...
OPSTATE ARY_Clear(volatile struct Array* ary){
	if (ARY_CheckError(ary)) return OPSTATE_FAILURE;
